...
let ytPreviewElement: HTMLDivElement | null = null;
...
export const showYTPreview = async (
  link: HTMLAnchorElement,
  event: MouseEvent
) => {
  if (!link.href.includes("youtube.com")) return;
  ...
  try {
    ...
    const oembedUrl =
      `https://www.youtube.com/oembed?url=${encodeURIComponent(link.href)}&format=json`;
...
export const hideYTPreview = () => {
  ...
  setTimeout(() => {
    if (ytPreviewElement) ytPreviewElement.style.display = "none";
  }, YT_PREVIEW_CONFIG.fadeDuration);
};

export const isYTPreviewVisible = () =>
  !!ytPreviewElement && ytPreviewElement.style.display === "block";

export const updateYTPreviewPosition = (event: MouseEvent) => {
  if (!ytPreviewElement) return;
