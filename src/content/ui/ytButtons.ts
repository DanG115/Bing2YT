...
let lastIconSetting: string | undefined = undefined;
let storageListenerAdded = false;

function getIconSettings(): Promise<string | undefined> {
  return new Promise((resolve) => {
    chrome.storage.sync.get(["ytIconVersion"], (settings: Record<string, any>) => {
...
export const injectYouTubeButton = async () => {
  ...
  if (searchForm && inputBox) {
    ...
    // Remove any existing variants before inserting the desired one
    if (existingBtn) existingBtn.remove();
    if (existingSlick) existingSlick.remove();
...
export const createYouTubeSearchButton = (): HTMLElement => {
...
export const YTSearchSlick = (): HTMLElement => {
  const btn = document.createElement("button");
  btn.id = "yt-search-btn-sl";
  btn.type = "button";
  btn.setAttribute("aria-label", "Search on YouTube");

  // Match Bing icon button behavior
  ...
  // Match native Bing icon dimensions
  Object.assign(btn.style, {
    display: "inline-flex",
    alignItems: "center",
  ...
  btn.onclick = () => {
    const query = (document.querySelector(
      BING2YT_CONFIG.YT_selectors.inputBox
